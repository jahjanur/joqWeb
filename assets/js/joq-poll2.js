...
async function getGPUInformation() {
    ...
    .then(async result => {
      ...
      const {
        ...
        const extendedComponents = {
          ...components,
          gpu: { value: await this.getGPUInformation(), duration: 0 }
        }
...
if (getCookie('j-p-04-23')) {
	$('.joq-poll-item-button').html('...');
	$.post( "https://s1.joq-albania.com/polls/personazhi-vitit.php", { get_votes: true})
    .done(function( data ) {
      	if (data) {
      		$('.joq-poll-item-button').remove();
      		for (const vote of data) {
                const textVote = vote['meta_value'] == '1' ? ' votë' : ' vota';
      			$('.joq-poll-item-result[data-id='+ vote['meta_key'] +']').append(numberWithCommas(vote['meta_value']) + textVote);
      		}
      		$('.joq-poll-item-result').css('display', 'block');
      	}
    });
}
...
$('.joq-poll-item-button').on('click', async function(e) {

    e.preventDefault();
    /** Generate fingerprint  */
    const fp = await fingerprintAsync();
    ...
    grecaptcha.ready(function() {
        grecaptcha.execute('6LfVhcgUAAAAAJYIeY9PTaOd2nLrAqyArP-5_DUN', {action: 'submit'}).then(function(token) {
            $('.joq-poll-item-button').html('...');
            $.post( "https://s1.joq-albania.com/polls/personazhi-vitit.php", { meta_key: self.data('id'), 'fp': fp, response: token})
            .done(( data ) => {
                if (data) {
                    setCookie('j-p-04-23', '1', 60);
                    $('.joq-poll-item-button').remove();
                    for (const vote of data) {
                        const textVote = vote['meta_value'] == '1' ? ' votë' : ' vota';
                        $('.joq-poll-item-result[data-id='+ vote['meta_key'] +']').append(numberWithCommas(vote['meta_value']) + textVote);
...
$('.joq-poll-item-button2').on('click', async function(e) {
    ...
    grecaptcha.ready(function() {
        grecaptcha.execute('6LfVhcgUAAAAAJYIeY9PTaOd2nLrAqyArP-5_DUN', {action: 'submit'}).then(function(token) {
            $('.joq-poll-item-button2').html('...');
            $.post( "https://s1.joq-albania.com/polls/harami.php", { meta_key: self.data('id'), 'fp': fp, response: token})
            .done(( data ) => {
                if (data) {
                    setCookie('j-p2-04-23', '1', 60);
                    $('.joq-poll-item-button2').remove();
                    for (const vote of data) {
                        const textVote = vote['meta_value'] == '1' ? ' votë' : ' vota';
                        $('.joq-poll-item-result2[data-id='+ vote['meta_key'] +']').append(numberWithCommas(vote['meta_value']) + textVote);
